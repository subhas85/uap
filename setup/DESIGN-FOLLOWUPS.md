...
- **Re-running `apply.sh xrdp` can leave xrdp stuck in `activating`.** The packaged unit is `Type=forking` with `PIDFile=/run/xrdp/xrdp.pid`, and on Ubuntu 24.04 systemd checks for the PID file before xrdp writes it — `Can't open PID file /run/xrdp/xrdp.pid (yet?) after start`. The daemon runs and serves 3389 normally, but systemd never adopts the main PID: `ActiveState=activating`, `ExecMainPID=0`, and it will sit there until `TimeoutStartSec`. Seen on a re-run of just the xrdp component; a plain `systemctl restart xrdp` afterwards settles it to `active`. Fix is probably a drop-in with `PIDFile=` removed (let systemd track the forked child) or `Type=simple` plus xrdp's `--nodaemon`. Until then `install_xrdp` should verify `systemctl is-active xrdp` at the end and warn if it isn't — right now it reports success regardless.
